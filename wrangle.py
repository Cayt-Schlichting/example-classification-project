...
def getNewData(ds_name,ds=ds):
    ...
    Inputs:
      (R) - ds_name: The name of the dataset you want to access. See supported datasets below.
    ...
    Supported datasets: 
      ...
      'telco'
    """
    #pull out information needed to perform SQL query
...
def getData(ds_name,ds=ds):
    ...
    Inputs:
      ...
      (O) -      ds: A Pandas dataframe of the following format:
                    index: corresponds to the ds_name provided
                    columns:  'filename': filename you want to store the data at and retrieve it from
                               'db_name': the name of the database in on the codeup server
                                   'sql': contains SQL query to executed
